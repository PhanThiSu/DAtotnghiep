		...
		<div class="col-xs-12">
			<div class="box">
			    <div class="box-header with-border row" id="reports-header">
					<div class="col-sm-6">
						<h3 class="box-title">
							Report each month in <?=$this->time?> of <?=vendor_html_helper::link($this->fullname, ["ctl"=>"users", "act"=>"view/".$this->user_id])?>
						</h3>
					</div>
					<div class="col-sm-6">
						<h3 class="box-title text-info pull-right text-right">In <a class="dayweek" alt="<?=$this->time;?>" href="<?=vendor_app_util::url(array('ctl'=>'reports', 'act'=>'user/'.$this->user_id)); ?>"><?=$this->time;?></a>, <?=vendor_html_helper::link($this->fullname, ["ctl"=>"users", "act"=>"view/".$this->user_id])?> have worked <strong class="text-danger"><?=$this->record['total'];?></strong> hour(s)</h3>
					</div>
			    </div>
									...
									<tbody id="tbody-reports">
						                ...
						                <?php foreach($this->record['data'] as $k=>$v) { ?>
						                	<tr>
												...
												<td><?=$v; ?></td>
												<td>
													<a type="button" class="btn btn-success view-record dayweek" alt="<?=date('Y-m-d', strtotime('+'.$i.' day', strtotime($this->monday)))?>" href="<?=vendor_app_util::url(array('ctl'=>'reports', 'act'=>'user/'.$this->user_id)); ?>"><i class="fa fa-eye" aria-hidden="true"></i>
													</a>
								                  	<a href="<?php echo (vendor_app_util::url(["ctl"=>"reports", "act"=>"userreports/".$this->user_id])) ?>" id="view<?=$this->user_id;?>" type="button" class="btn btn-success view-record">
...
<script src="<?php echo RootREL; ?>media/admin/js/userdays.js"></script>

		...
		<div class="col-xs-12">
			<div class="box">
			    <div class="box-header with-border row id="reports-header">
					...
					<div class="col-sm-6">
						<h3 class="box-title text-info pull-right text-right">In <?=$this->time;?>, <?=vendor_html_helper::link($this->fullname, ["ctl"=>"users", "act"=>"view/".$this->user_id])?> have worked <strong class="text-danger"><?=$this->record['total'];?></strong> hour(s)</h3>
					</div>
			    </div>
									...
									<tbody id="tbody-reports">
						                ...
						                <?php foreach($this->record['data'] as $k=>$v) { ?>
						                	<tr>
												...
												<td><?=$v; ?></td>
												<td>
													<a type="button" class="btn btn-success view-record month" alt="<?=$this->time.'-'.(($i<10)? '0':'').$i;?>" href="<?=vendor_app_util::url(array('ctl'=>'reports', 'act'=>'user/'.$this->user_id)); ?>"><i class="fa fa-eye" aria-hidden="true"></i>
													</a>
												</td>
						                	</tr>
											<?php $i++; ?>
...
<script src="<?php echo RootREL; ?>media/admin/js/usermonths.js"></script>

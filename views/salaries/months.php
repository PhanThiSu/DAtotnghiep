		...
		<div class="col-xs-12">
			<div class="box">
			    <div class="box-header with-border row id='reports-header' ">
					<div class="col-sm-6">
						<h3 class="box-title">
                             salaries per month in <?=$this->year?> <?= !empty($this->quarter) ? ", quarter (".$this->quarter.")":"" ?>  </a>
						</h3>
					</div>
					<div class="col-sm-6">
			    ...
			    <div class="box-body row">
					...
					<div class="col-sm-9 col-xs-9">
		      			...
		      			<form id="form-report" action="<?php echo vendor_app_util::url(["ctl"=>"salaries", "act"=>'months']); ?>" method="post" enctype="multipart/form-data" class="form-inline">
                            ...
                            <table id="table_reports" class="table table-bordered table-striped dataTable" role = "grid" aria-describedby="example1_info" controller="reports" >
                                ...
                                <?php if (isset($this->record)) { ?>
                                    ...
                                    <?php foreach($this->record as $k=>$v) { ?>
                                        ...
                                        <tr>
                                            <td><?= $this->year==0 ? intval($k):$k ?></td>
                                            <td><?=number_format($v);?></td>
                                            <td>
                                                <a type="button" class="btn btn-success view-record usermonth" alt="<?=$this->time.'-'.(($i<10)? '0':'').$i;?>" href="<?=vendor_app_util::url(array('ctl'=>'reports','act'=>'month?year='.$this->year.'&month='.$i)); ?>"><i class="fa fa-eye" aria-hidden="true"></i>
                                                </a>
                                            </td>
                                        </tr>
                                        <?php $i++; ?>

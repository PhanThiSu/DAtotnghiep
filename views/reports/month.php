...
<?php vendor_html_helper::contentheader('Báo cáo công việc <small></small>', [['urlp'=>['ctl'=>$app['ctl'], 'act'=>$app['act']]]]); ?>
		...
		<div class="col-xs-12">
			<div class="box">
				<div class="box-header with-border row" id="reports-header">
					<div class="col-sm-6">
						<h3 class="box-title">
							Báo cáo <?=$this->time?> của <?=vendor_html_helper::link($this->fullname, ["ctl"=>"reports", "act"=>"profile"])?>
						</h3>
					</div>
					<!-- <div class="col-sm-6">
						<h3 class="box-title text-info pull-right text-right">This Month <?=vendor_html_helper::link($this->fullname, ["ctl"=>"reports", "act"=>"profile"])?> has worked <strong class="text-danger"><?=number_format((float)$this->timetotal , 1, '.', '');?></strong> hour(s)</h3>
					</div> -->
			    </div>
			      			...
			      			<form id="form-report" action="<?php echo vendor_app_util::url(["ctl"=>"reports", "act"=>'month']); ?>" method="post" enctype="multipart/form-data" class="form-inline">
								<div class="form-group">
								    <div class="input-group">
								      	<div class="input-group-addon mobileHide">Chọn tháng: </div>
